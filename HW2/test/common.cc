...
// the non-printable character. (It also replaces a CR with <CR> and a LF with
// <LF>.)
...
void expectNoMoreData(struct connection *conn) {
  ...
  int r = read(conn->fd, &conn->buf[conn->bytesInBuffer],
               conn->bufferSizeBytes - conn->bytesInBuffer);
  ...
  if (conn->bytesInBuffer > 0) {
    log("S: ", conn->buf, conn->bytesInBuffer,
        " [unexpected; server should not have sent anything!]\n");
...
void expectToRead(struct connection *conn, const char *data) {
  ...
  while (true) {
    ...
    int bytesRead = read(conn->fd, &conn->buf[conn->bytesInBuffer],
                         conn->bufferSizeBytes - conn->bytesInBuffer);
    if (bytesRead < 0)
  ...
  // Get rid of the LF (or, if it is preceded by a CR, of both the CR and the
  // LF)
  ...
  // Annotate the output to indicate whether the response matched the
  // expectation.
  ...
  // 'Eat' the line we just parsed. However, keep in mind that there might still
  // be
  // more bytes in the buffer (e.g., another line, or a part of one), so we have
  // to
...
void expectRemoteClose(struct connection *conn) {
  ...
  if (r < 0)
    panic("Read failed (%s)", strerror(errno));
  if (r > 0) {
    log("S: ", conn->buf, r + conn->bytesInBuffer,
        " [unexpected; server should have closed the connection]\n");
    conn->bytesInBuffer = 0;
  }
}
